...
// 导入大实验组件
import LightReflectRefract from './components/experiments/LightReflectRefract';
import LensImaging from './components/experiments/LensImaging';
import BuoyancySimulator from './components/experiments/BuoyancySimulator';
import LeverSimulator from './components/experiments/LeverSimulator';

export default function App() {
  // 核心状态：锁定亮色模式 (Light mode only)
  const [activeTab, setActiveTab] = useState('study'); // study | formulas | diagnosis | wrongbook
  
  // 学习阶段状态：study (章节学习首页) | quiz (专注答题测试)
  const [chapterStep, setChapterStep] = useState('study');
  ...
  const [submittedAnswers, setSubmittedAnswers] = useState({}); // { [qId]: true/false }
  const [wrongList, setWrongList] = useState([]); // 错题列表，保存在 localStorage
  
  // 当前章节答题索引 (0 - 19)
  const [currentQuizIndex, setCurrentQuizIndex] = useState(0);
  const [selectedQuizOption, setSelectedQuizOption] = useState(null);
  const [quizChecked, setQuizChecked] = useState(false);
  const [quizScore, setQuizScore] = useState(null); // 该章完成后的正确题数
  ...
  // 当切换主要 Tab 时，如果有特训状态则予以清空，且重设答题页面
  useEffect(() => {
    if (activeTab !== 'study') {
  ...
  const handleClearAllWrongs = () => {
    if (window.confirm('您确定要清空错题本中所有的题目吗？')) {
      setWrongList([]);
  ...
  blocks.forEach(b => {
    const blockQIds = questions.filter(q => q.blockId === b.id).map(q => q.id);
    const attempted = blockQIds.some(qId => userAnswers[qId] !== undefined);
    if (attempted) hasAnyAttempt = true;
  ...
  // 12个章节首页左侧【中考重点公式精讲】与【核心规律避坑警示】卡片生成
  const renderChapterFormulaGuide = () => {
    const formulasMap = {
      chapter1: {
        title: '📐 本章中考重点公式：速度公式',
        latex: 'v = \\frac{s}{t}',
        derivatives: ['s = v t (计算路程)', 't = \\frac{s}{v} (计算时间)'],
        variables: [
      ...
      chapter2: {
        ...
        latex: 'v_{\\text{声}} = 340\\text{ m/s} \\quad (15^\\circ\\text{C 空气中})',
      ...
      chapter3: {
        ...
        latex: 'T_{\\text{熔化}} = T_{\\text{熔点}} \\quad \\& \\quad T_{\\text{沸腾}} = T_{\\text{沸点}}',
      ...
      chapter4: {
        ...
        latex: '\\angle r = \\angle i \\quad (\\text{光的反射角等于入射角})',
      ...
      chapter5: {
        ...
        latex: 'u > f \\text{ 成实像} \\quad / \\quad u < f \\text{ 成虚像}',
      ...
      chapter6: {
        title: '📐 本章中考重点公式：密度公式',
        latex: '\\rho = \\frac{m}{V}',
        derivatives: ['m = \\rho V (计算质量)', 'V = \\frac{m}{\\rho} (计算体积)'],
        variables: [
          { symbol: 'ρ (rho)', name: '密度', unit: 'kg/m³ 或 g/cm³ (换算：1 g/cm³ = 1.0×10³ kg/m³)' },
      ...
      chapter7: {
        ...
        latex: 'G = m g',
        derivatives: ['m = \\frac{G}{g} (由重力推导质量)', '物体受到的重力与其质量成【正比】'],
        variables: [
      ...
      chapter9: {
        ...
        latex: 'p = \\frac{F}{S} \\quad \\& \\quad p = \\rho g h',
      ...
      chapter11: {
        ...
        latex: 'W = F s \\quad \\& \\quad P = \\frac{W}{t} = F v',
      ...
      chapter12: {
        ...
        latex: 'F_1 L_1 = F_2 L_2 \\quad \\& \\quad F = \\frac{1}{n}(G_{\\text{物}} + G_{\\text{动}})',
  ...
  // 生成 12 个章节，每个章节 2 幅精美 SVG 原生插图 (去除 glass-card 层，扁平化小面板)
    ...
    if (selectedChapterId === 'chapter1') {
      ...
      list.push(
        <div key="c1-2" style={{ padding: '16px', backgroundColor: '#f8fafc', border: '1px solid #edf2f7', borderRadius: 'var(--radius-md)' }}>
          <div style={{ fontSize: '0.78rem', color: 'hsl(var(--text-secondary))', marginBottom: '8px', fontWeight: 'bold' }}>🖼️ 图解 1.2：平均速度计算 (v_平 = s_总 / t_总)</div>
          <svg width="100%" height="110" viewBox="0 0 400 110" style={{ display: 'block', margin: '0 auto', maxWidth: '400px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: '10px', alignItems: 'center' }}>
            ...
            <div style={{ fontSize: '0.75rem', backgroundColor: '#ffffff', padding: '8px', borderRadius: '6px', minHeight: '80px', display: 'flex', flexDirection: 'column', justifyContent: 'center', border: '1px solid #edf2f7' }}>
              {activeStateChange ? (
                ...
                <div style={{ opacity: 0.6, textAlign: 'center', fontSize: '0.68rem' }}>点击左侧箭头查看物态变化吸放热</div>
              )}
            </div>
          </div>
        </div>
      ...
      list.push(
        <div key="c4-1" style={{ padding: '16px', backgroundColor: '#f8fafc', border: '1px solid #edf2f7', borderRadius: 'var(--radius-md)' }}>
          ...
          <svg width="100%" height="110" viewBox="0 0 400 110" style={{ display: 'block', margin: '0 auto', maxWidth: '400px' }}>
            ...
            <path d="M 32.5 45 Q 28 35 32.5 25 Q 37 35 32.5 45" fill="url(#candleFlame)" />
            ...
            <path d="M 330 65 Q 326 73 330 81 Q 334 73 330 65" fill="url(#candleFlame)" transform="rotate(180, 330, 65)" />
    ...
    if (selectedChapterId === 'chapter6') {
      ...
      list.push(
        <div key="c6-2" style={{ padding: '16px', backgroundColor: '#f8fafc', border: '1px solid #edf2f7', borderRadius: 'var(--radius-md)' }}>
          <div style={{ fontSize: '0.78rem', color: 'hsl(var(--text-secondary))', marginBottom: '8px', fontWeight: 'bold' }}>🖼️ 图解 6.2：质量-体积图像 (m-V) 物理斜率即密度</div>
          <svg width="100%" height="110" viewBox="0 0 400 110" style={{ display: 'block', margin: '0 auto', maxWidth: '400px' }}>
      ...
      list.push(
        <div key="c7-1" style={{ padding: '16px', backgroundColor: '#f8fafc', border: '1px solid #edf2f7', borderRadius: 'var(--radius-md)' }}>
          ...
          <svg width="100%" height="110" viewBox="0 0 400 110" style={{ display: 'block', margin: '0 auto', maxWidth: '400px' }}>
            ...
            <path d="M 340 55 Q 355 55 350 65 Q 345 75 355 75" fill="none" stroke="#4a5568" strokeWidth="2.5" />
      ...
      list.push(
        <div key="c7-2" style={{ padding: '16px', backgroundColor: '#f8fafc', border: '1px solid #edf2f7', borderRadius: 'var(--radius-md)' }}>
          ...
          <svg width="100%" height="110" viewBox="0 0 400 110" style={{ display: 'block', margin: '0 auto', maxWidth: '400px' }}>
            ...
            <text x="110" y="86" fill="#718096" fontSize="8" fontWeight="bold">❌ 错误：垂直斜面向下</text>
          </svg>
        </div>
      );
    }
    ...
    if (selectedChapterId === 'chapter8') {
      list.push(
        <div key="c8-1" style={{ padding: '16px', backgroundColor: '#f8fafc', border: '1px solid #edf2f7', borderRadius: 'var(--radius-md)' }}>
          <div style={{ fontSize: '0.78rem', color: 'hsl(var(--text-secondary))', marginBottom: '8px', fontWeight: 'bold' }}>🖼️ 图解 8.1：探究阻力对小车运动的影响 (牛顿第一定律基础)</div>
          <svg width="100%" height="110" viewBox="0 0 400 110" style={{ display: 'block', margin: '0 auto', maxWidth: '400px' }}>
    ...
    if (selectedChapterId === 'chapter9') {
      list.push(
        <div key="c9-1" style={{ padding: '16px', backgroundColor: '#f8fafc', border: '1px solid #edf2f7', borderRadius: 'var(--radius-md)' }}>
          <div style={{ fontSize: '0.78rem', color: 'hsl(var(--text-secondary))', marginBottom: '8px', fontWeight: 'bold' }}>🖼️ 图解 9.1：探究压力的作用效果影响因素 (海绵小桌实验)</div>
          <svg width="100%" height="110" viewBox="0 0 400 110" style={{ display: 'block', margin: '0 auto', maxWidth: '400px' }}>
            ...
            <text x="80" y="10" fill="hsl(var(--color-danger))" fontSize="8.5" fontWeight="bold" textAnchor="middle">A: 阻力面陷得深</text>
            ...
            <svg width="100%" height="90" viewBox="0 0 200 90" style={{ maxWidth: '200px' }}>
              ...
              {pendulumPos === 'top' && (
                <>
                  ...
                  <circle cx="40" cy="65" r="8" fill="url(#ballGlowLight)" stroke="hsl(var(--color-heat))" strokeWidth="1.5" />
              ...
              {pendulumPos === 'mid' && (
                <>
                  ...
                  <circle cx="70" cy="78" r="8" fill="url(#ballGlowLight)" stroke="hsl(var(--color-optics))" strokeWidth="1.5" />
              ...
              {pendulumPos === 'bottom' && (
                <>
                  ...
                  <circle cx="100" cy="83" r="8" fill="url(#ballGlowLight)" stroke="hsl(var(--color-mech))" strokeWidth="1.5" />
    ...
    if (selectedChapterId === 'chapter12') {
      list.push(
        <div key="c12-1" style={{ padding: '16px', backgroundColor: '#f8fafc', border: '1px solid #edf2f7', borderRadius: 'var(--radius-md)' }}>
          <div style={{ fontSize: '0.78rem', color: 'hsl(var(--text-secondary))', marginBottom: '8px', fontWeight: 'bold' }}>🖼️ 图解 12.1：滑轮组省力公式与绳子股数 n 判定分析</div>
          <svg width="100%" height="120" viewBox="0 0 400 120" style={{ display: 'block', margin: '0 auto', maxWidth: '400px' }}>
    ...
    <div className="app-container">
      ...
      <div className="sidebar">
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <div>
            <h2 style={{ fontSize: '1.05rem', border: 'none', padding: 0, margin: 0, letterSpacing: '0.5px' }}>中考物理基础</h2>
            <span style={{ fontSize: '0.75rem', opacity: 0.6 }}>八年级上下册冲刺宝典</span>
          </div>
        </div>
      ...
      <div className="main-content">
        ...
        {activeTab === 'study' && (
          <div className="fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            ...
            {chapterStep === 'study' && (
              <>
                <div className="glass-card" style={{ padding: '12px', display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                  {blocks.map(b => (
                    <button
                      key={b.id}
                      className={`btn ${selectedBlockId === b.id ? 'btn-primary' : 'btn-secondary'}`}
                      style={{
                        fontSize: '0.85rem',
                        padding: '8px 16px',
                        borderColor: selectedBlockId === b.id ? '' : 'hsla(var(--text-secondary) / 0.1)',
                        backgroundColor: selectedBlockId === b.id ? b.color : '',
                        color: selectedBlockId === b.id ? '#fff' : ''
                      }}
                      onClick={() => {
                        setSelectedBlockId(b.id);
                        setSelectedChapterId(b.chapters[0]);
                      }}
                    >
                      {b.name}
                    </button>
                ...
                <div style={{ display: 'flex', gap: '8px', overflowX: 'auto', paddingBottom: '4px' }}>
                  {blocks.find(b => b.id === selectedBlockId).chapters.map(chId => {
                    const ch = chapters[chId];
                    return (
                      <button
                        key={chId}
                        className={`btn ${selectedChapterId === chId ? 'btn-accent' : 'btn-secondary'}`}
                        style={{
                          fontSize: '0.8rem',
                          padding: '6px 14px',
                          borderRadius: '20px',
            ...
            {/* 学习首页模式：工作台等高双栏布局（Equal-Height Layout，固定高度且内部顺畅滚动） */}
            {chapterStep === 'study' ? (
              <div className="fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                ...
                <div style={{
                  ...
                  <div className="glass-card" style={{
                    ...
                    flexDirection: 'column',
                    height: '100%',
                    borderTop: '4px solid var(--color-mech)',
                    backgroundColor: '#fff'
                  }}>
                    {/* 卡片头部（固定，不滚动） */}
                    <div style={{ padding: '20px 24px 12px 24px', borderBottom: '1px solid #edf2f7' }}>
                      <span className="badge badge-mech" style={{ marginBottom: '6px' }}>重点知识干货精讲</span>
                      <h3 style={{ fontSize: '1.2rem' }}>{chapters[selectedChapterId].name}</h3>
                    </div>
                    ...
                    <div style={{
                      ...
                      {chapters[selectedChapterId].summary.split('\n').map((line, idx) => {
                        const processText = (text) => {
                          const parts = text.split('**');
                          return parts.map((part, i) => i % 2 === 1 ? <strong key={i} style={{ color: 'hsl(var(--color-optics))' }}>{part}</strong> : part);
                        };
                        return (
                  ...
                  {/* 右侧大卡片：公式警示卡片 + 物理微图解画廊 + 互动实验室 */}
                  <div className="glass-card" style={{
                    display: 'flex',
                    flexDirection: 'column',
                    height: '100%',
                    borderTop: '4px solid var(--color-work)',
                    backgroundColor: '#fff'
                  }}>
                    {/* 卡片头部 (固定) */}
                    <div style={{ padding: '20px 24px 12px 24px', borderBottom: '1px solid #edf2f7' }}>
                      <span className="badge badge-work" style={{ marginBottom: '6px' }}>中考公式与物理模型画廊</span>
                      <h3 style={{ fontSize: '1.2rem' }}>公式讲解及图解实验区</h3>
                    </div>
                ...
                <button
                  ...
                  style={{
                    ...
                    fontSize: '1.05rem',
                    borderRadius: '30px',
                    boxShadow: '0 8px 20px hsla(var(--color-mech)/0.2)',
                    width: '100%',
                    marginTop: '8px',
                    fontWeight: 'bold',
                    letterSpacing: '1px'
                  }}
                  onClick={() => setChapterStep('quiz')}
                >
                  🚀 知识点已学完，开始本章中考基础闯关 (20题) 🚀
                </button>
              </div>
            ) : (
              // 专注答题模式
              <div className="fade-in" style={{ maxWidth: '780px', margin: '0 auto', width: '100%' }}>
                <button
                  ...
                  style={{ marginBottom: '16px', padding: '6px 14px', fontSize: '0.82rem' }}
                  onClick={() => setChapterStep('study')}
                >
                  🔙 返回本章首页重新学习
                </button>

                <div className="glass-card" style={{ padding: '28px', borderTop: '4px solid var(--color-work)', backgroundColor: '#fff' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                    <div>
                      <span className="badge badge-work" style={{ marginRight: '8px' }}>基础闯关测试</span>
                      <span style={{ fontSize: '0.85rem', fontWeight: 'bold', color: 'hsl(var(--color-mech))' }}>
                        {chapters[selectedChapterId].name}
                      </span>
                    </div>
                    <span style={{ fontSize: '0.82rem', opacity: 0.7, fontFamily: 'monospace', fontWeight: 'bold' }}>
                      进度：{currentQuizIndex + 1} / {currentChapterQuestions.length}
                    </span>
                  </div>

                  <div style={{ width: '100%', height: '5px', backgroundColor: '#edf2f7', borderRadius: '4px', overflow: 'hidden', marginBottom: '20px' }}>
                    <div style={{
                      width: `${((currentQuizIndex) / currentChapterQuestions.length) * 100}%`,
                      height: '100%',
                      backgroundColor: 'hsl(var(--color-mech))',
                  ...
                  {quizScore === null ? (
                    <div className="fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                      {currentChapterQuestions[currentQuizIndex] && (
                        <>
                          <div style={{ fontSize: '0.8rem', color: 'hsl(var(--color-mech))', fontWeight: 'bold' }}>
                            📌 考点：{currentChapterQuestions[currentQuizIndex].category}
                          </div>
                          <div style={{ fontSize: '1rem', fontWeight: 600, lineHeight: '1.6', color: 'hsl(var(--text-primary))' }}>
                            {currentChapterQuestions[currentQuizIndex].question}
                          </div>
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginTop: '8px' }}>
                            {currentChapterQuestions[currentQuizIndex].options.map((opt, oIdx) => {
                              ...
                              let optBorder = '';
                              if (isSelected) {
                                optBg = 'hsla(var(--color-mech) / 0.08)';
                                optBorder = '1px solid hsl(var(--color-mech))';
                              }

                              if (quizChecked) {
                                if (oIdx === currentQ.answer) {
                                  optBg = 'hsla(var(--color-success) / 0.12)';
                                  optBorder = '1px solid hsl(var(--color-success))';
                                } else if (isSelected) {
                                  optBg = 'hsla(var(--color-danger) / 0.12)';
                                  optBorder = '1px solid hsl(var(--color-danger))';
                                }
                              }
                              ...
                              return (
                                <button
                                  ...
                                  style={{
                                    ...
                                    padding: '12px 16px',
                                    fontSize: '0.88rem',
                                    backgroundColor: optBg || '#fff',
                                    border: optBorder || '1px solid hsl(var(--border))',
                                    color: 'hsl(var(--text-primary))',
                                    cursor: quizChecked ? 'default' : 'pointer'
                                  }}
                                  onClick={() => setSelectedQuizOption(oIdx)}
                                >
                                  <strong style={{ marginRight: '8px', color: isSelected ? 'hsl(var(--color-mech))' : 'hsl(var(--text-secondary))' }}>
                          ...
                          {quizChecked && (
                            <div className="scale-up" style={{
                              padding: '16px',
                              borderRadius: 'var(--radius-md)',
                              backgroundColor: selectedQuizOption === currentChapterQuestions[currentQuizIndex].answer ? 'rgba(56, 161, 105, 0.05)' : 'rgba(229, 62, 62, 0.05)',
                              border: selectedQuizOption === currentChapterQuestions[currentQuizIndex].answer ? '1px solid rgba(56, 161, 105, 0.2)' : '1px solid rgba(229, 62, 62, 0.2)',
                              marginTop: '10px'
                            }}>
                              <div style={{
                                fontWeight: 'bold',
                                color: selectedQuizOption === currentChapterQuestions[currentQuizIndex].answer ? 'hsl(var(--color-success))' : 'hsl(var(--color-danger))',
                                fontSize: '0.9rem',
                                marginBottom: '6px'
                              }}>
                                {selectedQuizOption === currentChapterQuestions[currentQuizIndex].answer ? '🎉 回答正确！思路很清晰！' : '❌ 回答错误！检测到基础漏洞。'}
                              </div>
                              <div style={{ fontSize: '0.85rem', opacity: 0.95, lineHeight: '1.6', color: 'hsl(var(--text-primary))' }}>
                          ...
                          <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '12px' }}>
                            {!quizChecked ? (
                              ...
                              >
                                {currentQuizIndex === currentChapterQuestions.length - 1 ? '完成测试，查看过关报告' : '进入下一题'}
                              </button>
                            )}
                          </div>
                        </>
                    ...
                    <div className="scale-up" style={{ display: 'flex', flexDirection: 'column', gap: '18px', textAlign: 'center', padding: '10px 0' }}>
                      <div style={{ fontSize: '3.5rem' }}>
                        {quizScore >= 16 ? '🏆' : quizScore >= 12 ? '💪' : '📚'}
                      </div>
                      <h3 style={{ fontSize: '1.25rem' }}>本章闯关已结束！</h3>
                      <div style={{ fontSize: '1.1rem', fontWeight: 'bold' }}>
                        正确率：<span style={{ color: quizScore >= 16 ? 'hsl(var(--color-success))' : 'hsl(var(--color-warning))' }}>{quizScore * 5}%</span> ({quizScore}/20 正确)
                      </div>
                      <p style={{ fontSize: '0.85rem', opacity: 0.8, padding: '0 15px', lineHeight: '1.6' }}>
                        {quizScore === 20 ? '恭喜您拿到 100 分满分！本章概念已被您彻底吃透，继续保持！' :
                         quizScore >= 16 ? '优秀水平！错题已收录入错题本，建议抽空消灭错题。' :
                         quizScore >= 12 ? '达到及格线，但仍存在不少物理考点盲区，建议重新返回学习首页对照原理图温习。' :
                         '正确率低于 60% 基础不够扎实。请务必返回本章首页阅读精讲，调试交互插图！'}
                      </p>
                      
                      <div style={{ display: 'flex', gap: '10px', marginTop: '16px', flexWrap: 'wrap' }}>
                        <button className="btn btn-secondary" style={{ flex: 1, padding: '10px', minWidth: '120px' }} onClick={() => setChapterStep('study')}>
                          📖 返回重新学习
                        </button>
                        <button className="btn btn-secondary" style={{ flex: 1, padding: '10px', minWidth: '120px' }} onClick={handleRestartQuiz}>
                          🔄 重新挑战答题
                        </button>
                        <button className="btn btn-primary" style={{ flex: 1, padding: '10px', minWidth: '120px' }} onClick={() => { setActiveTab('diagnosis'); }}>
                          📊 查看学情诊断
                        </button>
                      </div>
                    </div>
            ...
            {!isSpecialTraining ? (
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.2fr', gap: '24px', alignItems: 'start' }}>
                ...
                <div className="glass-card" style={{ padding: '20px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                  <span className="badge badge-mech" style={{ alignSelf: 'flex-start', marginBottom: '10px' }}>数据诊断</span>
                  <h3 style={{ fontSize: '1.2rem', width: '100%' }}>物理掌握度五维模型</h3>
                  <p style={{ fontSize: '0.8rem', opacity: 0.6, width: '100%', marginBottom: '12px' }}>
                    通过已答过的题目正确率与覆盖度多维计算得出雷达分。
                  </p>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                  ...
                  <div className="glass-card" style={{ padding: '24px', borderTop: '4px solid var(--color-optics)' }}>
                    <span className="badge badge-optics" style={{ marginBottom: '10px' }}>弱点报告</span>
                    <h3 style={{ fontSize: '1.25rem', marginBottom: '12px' }}>中考名师复习诊断建议</h3>
                    
                    {!hasAnyAttempt ? (
                      <div style={{ fontSize: '0.88rem', opacity: 0.8, lineHeight: '1.6', padding: '16px 0', textAlign: 'center' }}>
                        💡 <b>提示：</b>您当前还没有进行任何章节的基础闯关测试。请返回“📖 基础干货与实验”完成任意一章的测试题，系统便能为您生成精准的雷达掌握度模型并提供薄弱点建议。
                      </div>
                    ) : (
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', fontSize: '0.9rem' }}>
                        <div style={{
                          ...
                          🚨 <b>当前最薄弱的物理板块：</b>
                          <span style={{ fontWeight: 'bold', color: 'hsl(var(--color-heat))', marginLeft: '6px' }}>
                            {weakestBlock ? weakestBlock.name : '计算中'}
                          </span> (掌握度仅 {weakestBlock ? blockScores[weakestBlock.id] : 0}%)
                        </div>

                        <div style={{ lineHeight: '1.6' }}>
                          👉 <b>名师提分攻略：</b><br />
                          {weakestBlock?.id === 'block1' && '您的“声与光”板块得分较低。凸透镜成像规律是中考极其核心的实验题和填空题考点。建议前往第五章，在学习首页使用“凸透镜成像规律模拟器”拖动蜡烛多演练几遍，牢记“物近像远像变大，u=f是分界，2f点等大”的推论，拿稳基础分。'}
                          {weakestBlock?.id === 'block2' && '您的“物态与热学”板块是瓶颈。物态变化的核心考点在于六种变化的吸放热辨析。记住，“白气”、“白雾”都是液化形成的微小水滴，液化放热。温度计读数时视线必须与凹液面齐平，不能离开液体。'}
                          {weakestBlock?.id === 'block3' && '您的“运动与密度”部分基础还需巩固。长度估读必须写到分度值的下一位（例如刻度尺分度值为1mm，读数必须有两位小数厘米）。平均速度计算公式永远是“总路程除以总时间”，千万不能直接把两个速度加起来除以2！'}
                          {weakestBlock?.id === 'block4' && '力、压强与浮力是八年级最难的物理硬骨头。但中考偏题少，基础才是王道。压强 p=F/S 核心看受力面积S是不是实际接触面积；液体压强 p=ρgh 的深度 h 必须是到自由液面的垂直高度；浮力分析中，完全浸入水下后浮力不再改变！建议用浮力分析仪实验深入研究。'}
                          {weakestBlock?.id === 'block5' && '简单机械与功板块公式偏多，极易混淆。记住“有用功是目的（通常是提升重物 G*h）”，“总功是人付出的（拉力 F*s）”。任何机械都不能省功！功率大代表做功快，不代表做功多。多看看第十二章杠杆平衡条件实验。'}
                        </div>

                        {weakestBlock && (
                          <div style={{ marginTop: '10px' }}>
                            <button className="btn btn-primary" style={{ width: '100%', padding: '12px' }} onClick={() => startSpecialTraining(weakestBlock.id)}>
                              🔥 针对薄弱点：一键特训通关 (随机5道)
                            </button>
                          </div>
                        )}
                  ...
                  {hasAnyAttempt && (
                    <div className="glass-card" style={{ padding: '20px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', fontSize: '0.85rem' }}>
                      <div style={{ textAlign: 'center', borderRight: '1px solid rgba(0,0,0,0.06)' }}>
                        <div style={{ opacity: 0.6 }}>总答题进度</div>
                        <div style={{ fontSize: '1.4rem', fontWeight: 'bold', marginTop: '4px', color: 'hsl(var(--color-mech))' }}>
                          {Object.keys(userAnswers).length} / 240 <span style={{ fontSize: '0.8rem', fontWeight: 'normal' }}>道</span>
                        </div>
                      </div>
                      <div style={{ textAlign: 'center' }}>
                        <div style={{ opacity: 0.6 }}>整体做题正确率</div>
                        <div style={{ fontSize: '1.4rem', fontWeight: 'bold', marginTop: '4px', color: 'hsl(var(--color-success))' }}>
                          {(() => {
                            const total = Object.keys(userAnswers).length;
                            const correct = Object.values(submittedAnswers).filter(v => v === true).length;
              ...
              <div className="glass-card scale-up" style={{ padding: '24px', maxWidth: '600px', margin: '0 auto', borderTop: '4px solid var(--color-work)', backgroundColor: '#fff' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                  <div>
                    <span className="badge badge-work">🎯 薄弱针对性特训</span>
                    <h3 style={{ fontSize: '1.15rem', marginTop: '4px' }}>
                      正在特训：{blocks.find(b => b.id === specialTrainBlockId)?.name}
                    </h3>
                  </div>
                  <span style={{ fontSize: '0.8rem', opacity: 0.6, fontFamily: 'monospace' }}>
                    特训：{specialTrainIndex + 1} / 5
                  </span>
                </div>

                {specialTrainQuestions[specialTrainIndex] && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                    <div style={{ fontSize: '0.78rem', color: 'hsl(var(--color-work))', fontWeight: 'bold' }}>
                      📌 特训考点：{specialTrainQuestions[specialTrainIndex].category}
                    </div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                      {specialTrainQuestions[specialTrainIndex].options.map((opt, oIdx) => {
                        const isSelected = specialTrainSelectedOpt === oIdx;
                        const currentQ = specialTrainQuestions[specialTrainIndex];
                        
                        let optBg = '';
                        let optBorder = '';
                        if (isSelected) {
                          optBg = 'hsla(var(--color-work) / 0.1)';
                          optBorder = '1px solid hsl(var(--color-work))';
                        }

                        if (specialTrainChecked) {
                          if (oIdx === currentQ.answer) {
                            optBg = 'hsla(var(--color-success) / 0.15)';
                            optBorder = '1px solid hsl(var(--color-success))';
                          } else if (isSelected) {
                            optBg = 'hsla(var(--color-danger) / 0.15)';
                            optBorder = '1px solid hsl(var(--color-danger))';
                          }
                        }
                        ...
                        return (
                          <button
                            ...
                            style={{
                              ...
                              border: optBorder || '1px solid hsl(var(--border))',
                              color: 'hsl(var(--text-primary))',
                              cursor: specialTrainChecked ? 'default' : 'pointer'
                            }}
                            onClick={() => setSpecialTrainSelectedOpt(oIdx)}
                          >
                            <strong style={{ marginRight: '6px' }}>{String.fromCharCode(65 + oIdx)}.</strong>
                            {opt}
                          </button>
                        );
                      })}
                    ...
                    {specialTrainChecked && (
                      <div className="scale-up" style={{
                        padding: '12px',
                        borderRadius: 'var(--radius-sm)',
                        backgroundColor: specialTrainSelectedOpt === specialTrainQuestions[specialTrainIndex].answer ? 'hsla(var(--color-success) / 0.04)' : 'hsla(var(--color-danger) / 0.04)',
                        border: specialTrainSelectedOpt === specialTrainQuestions[specialTrainIndex].answer ? '1px solid hsla(var(--color-success) / 0.2)' : '1px solid hsla(var(--color-danger) / 0.2)'
                      }}>
                        <div style={{
                          fontWeight: 'bold',
                          color: specialTrainSelectedOpt === specialTrainQuestions[specialTrainIndex].answer ? 'hsl(var(--color-success))' : 'hsl(var(--color-danger))',
                          fontSize: '0.85rem',
                          marginBottom: '6px'
                        }}>
                          {specialTrainSelectedOpt === specialTrainQuestions[specialTrainIndex].answer ? '🎉 答对了！' : '❌ 答错了！'}
                        </div>
                    ...
                    <div style={{ display: 'flex', gap: '10px', marginTop: '10px' }}>
                      {!specialTrainChecked ? (
                        <>
                          <button className="btn btn-secondary" style={{ flex: 1 }} onClick={() => setIsSpecialTraining(false)}>
                            退出特训
                          </button>
                          <button className="btn btn-primary" style={{ flex: 2 }} disabled={specialTrainSelectedOpt === null} onClick={handleSpecialTrainSubmit}>
                            确认提交
                          </button>
                        </>
                      ) : (
                        <button className="btn btn-accent" style={{ width: '100%' }} onClick={handleSpecialTrainNext}>
                          {specialTrainIndex === 4 ? `特训结束 (答对 ${specialTrainScore} 道)` : '下一题'}
                        </button>
                      )}
                    </div>
                  </div>
      ...
      <svg style={{ position: 'absolute', width: 0, height: 0 }} aria-hidden="true">
        <defs>
          <marker id="ill-arrow" viewBox="0 0 10 10" refX="5" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
